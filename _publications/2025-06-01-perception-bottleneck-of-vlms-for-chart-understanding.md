---
title: "On the Perception Bottleneck of VLMs for Chart Understanding"
collection: publications
permalink: /publication/2025-perception-bottleneck-vlms-chart-understanding
venue: 'arXiv'
date: 2025-06-01
excerpt: '2025 arXiv paper (first author). Code is available on GitHub (Vision4Chart).'
citation: 'Junteng Liu, Weihao Zeng, Xiwen Zhang, Yijun Wang, Zifei Shan, Junxian He. (2025). "On the Perception Bottleneck of VLMs for Chart Understanding." <i>arXiv</i>.'
---
First-author paper on the perception bottleneck of vision-language models for chart understanding, published on arXiv in 2025. Code is available on GitHub (Vision4Chart).
